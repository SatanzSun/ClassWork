    ...
    public class Program
    {
       public static void Main(string[] args)
        ...
        private static bool GameFinished(FieldEnum[,] board)
        {
            ...
            {
                if (board[i, 0] != FieldEnum.Empty && board[i, 0] == board[i, 1] && board[i, 1] == board[i, 2])
                {
                    Console.WriteLine($"Uzvarētājs: {board[i, 0]}");
                    return true;
                }
            }
            for (int i = 0; i < board.GetLength(0); i++)
            {
                if (board[0, i] != FieldEnum.Empty && board[0, i] == board[1, 1] && board[1, i] == board[2, i])
                {
                    Console.WriteLine($"Uzvarētājs: {board[0, i]}");
                    return true;
                }
            }
            for (int i = 0; i < board.GetLength(0); i++)
            ...
            {
                Console.WriteLine($"Uzvarētājs: {board[0, 0]}");
                return true;
            }
            if(board[2, 2] != FieldEnum.Empty && board[2, 2] == board[1, 1] && board[1, 1] == board[0, 0])
            {
                Console.WriteLine($"Uzvarētājs: {board[0, 2]}");
                return true;
            }
            for (int i = 0; i < board.GetLength(0); i++)
            {
